...
<template>
    <AdminLayout>
        <template #header>
            <h1 class="text-lg font-semibold text-gray-900">Encadrants</h1>
        </template>
        ...
        <div class="space-y-4">
            ...
            >
                <template #actions>
                    <Link :href="route('admin.encadrants.export')" class="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-gray-200 text-gray-600 transition hover:bg-gray-50" title="Exporter">
            ...
            <div class="grid gap-4 rounded-xl border border-gray-200 bg-white p-4 lg:grid-cols-2">
                <div class="rounded-xl border border-gray-100 bg-gray-50 px-4 py-3">
                    <p class="text-xs text-gray-500">Total encadrants</p>
                    <p class="text-lg font-semibold text-gray-900">{{ stats?.total ?? 0 }}</p>
                </div>
                <div class="rounded-xl border border-gray-100 bg-gray-50 px-4 py-3">
                    <p class="text-xs text-gray-500">Avec theses</p>
                    <p class="text-lg font-semibold text-ed-primary">{{ stats?.avec_theses ?? 0 }}</p>
                </div>
            </div>
            ...
            <div class="rounded-xl border border-gray-200 bg-white p-4">
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:flex lg:flex-wrap lg:items-end">
                    <div class="sm:col-span-2 lg:flex-1 lg:min-w-[200px]">
                        ...
                        <input v-model="search" type="text" placeholder="Nom, email, grade, specialite" class="mt-2 w-full rounded-xl border border-gray-200 px-3 py-2 text-sm" />
                    </div>
                    <div class="w-full sm:w-auto">
                        <label class="text-xs font-semibold text-gray-600">Grade</label>
                        <select v-model="grade" class="mt-2 w-full rounded-xl border border-gray-200 px-3 py-2 text-sm">
            ...
            <div class="rounded-xl border border-gray-200 bg-white p-4">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    <div>
                        <p class="text-xs font-semibold text-gray-600">Importer des encadrants</p>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="divide-y divide-gray-100 text-sm">
                            <tr v-for="encadrant in encadrants.data" :key="encadrant.id" class="hover:bg-gray-50">
                                <td class="px-5 py-4">
                                    <p class="font-semibold text-gray-900">{{ encadrant.name }}</p>
                                    <p class="text-xs text-gray-500">{{ encadrant.email || '-' }}</p>
                                </td>
                                <td class="px-5 py-4 text-gray-600">
                                    <div class="flex flex-wrap items-center gap-2">
                                        <span>{{ encadrant.grade || '-' }}</span>
                                        <span v-if="!encadrant.grade" class="inline-flex items-center rounded-full border border-gray-200 bg-gray-50 px-2 py-0.5 text-xs font-semibold text-gray-600">
                                            Grade manquant
                                        </span>
                                    </div>
                                </td>
                                <td class="px-5 py-4 text-gray-600">{{ encadrant.specialite || '-' }}</td>
                                <td class="px-5 py-4 text-gray-600">
                                ...
                                <td class="px-5 py-4 text-right">
                                    <div class="flex justify-end gap-1">
                                        <Link :href="route('admin.encadrants.show', encadrant.id)" class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-gray-200 text-gray-600 transition hover:bg-gray-50" title="Voir">
                                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.477 0 8.268 2.943 9.542 7-1.274 4.057-5.065 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                            </svg>
                                        </Link>
                                        <Link :href="route('admin.encadrants.edit', encadrant.id)" class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-gray-200 text-gray-600 transition hover:border-ed-primary hover:bg-ed-primary/5 hover:text-ed-primary" title="Modifier">
                                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <button type="button" class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-gray-200 text-gray-600 transition hover:border-red-300 hover:bg-red-50 hover:text-red-600" title="Supprimer" @click="deleteEncadrant(encadrant)">
                                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <tr v-if="!encadrants.data.length">
                                <td colspan="5" class="px-6 py-8 text-center text-sm text-gray-500">Aucun encadrant trouve.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                <div v-if="encadrants.links?.length" class="border-t border-gray-100 px-4 py-3">
