...
<template>
    <AdminLayout>
        <template #header>
            <h1 class="text-lg font-semibold text-gray-900">Specialites</h1>
        </template>
        ...
        <div class="space-y-4">
            ...
            <div class="grid gap-4 rounded-xl border border-gray-200 bg-white p-4 lg:grid-cols-3">
                <div class="rounded-xl border border-gray-100 bg-gray-50 px-4 py-3">
                    <p class="text-xs text-gray-500">Total specialites</p>
                    <p class="text-lg font-semibold text-gray-900">{{ stats?.total ?? 0 }}</p>
                </div>
                <div class="rounded-xl border border-gray-100 bg-gray-50 px-4 py-3">
                    <p class="text-xs text-gray-500">Avec EAD</p>
                    <p class="text-lg font-semibold text-ed-primary">{{ stats?.avec_ead ?? 0 }}</p>
                </div>
                <div class="rounded-xl border border-gray-100 bg-gray-50 px-4 py-3">
            ...
            <div class="flex flex-wrap items-end gap-3 rounded-xl border border-gray-200 bg-white p-4">
                <div class="flex-1 min-w-[200px]">
                    ...
                    <input v-model="search" type="text" placeholder="Nom, slug, description..." class="mt-2 w-full rounded-xl border border-gray-200 px-3 py-2 text-sm" />
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="divide-y divide-gray-100 text-sm">
                            <tr v-for="specialite in specialites.data" :key="specialite.id" class="hover:bg-gray-50">
                                <td class="px-5 py-4">
                                    <p class="font-semibold text-gray-900">{{ specialite.nom }}</p>
                                    <p class="text-xs text-gray-500">/{{ specialite.slug }}</p>
                                </td>
                                <td class="px-5 py-4 text-gray-600">{{ specialite.ead?.nom || '-' }}</td>
                                ...
                                <td class="px-5 py-4 text-right">
                                    <div class="flex justify-end gap-2">
                                        <button type="button" class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-gray-200 text-gray-600 transition hover:border-ed-primary hover:bg-ed-primary/5 hover:text-ed-primary" title="Modifier" @click="openEdit(specialite)">
                                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <button type="button" class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-gray-200 text-gray-600 transition hover:border-red-300 hover:bg-red-50 hover:text-red-600" title="Supprimer" @click="deleteSpecialite(specialite)">
                                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <tr v-if="!specialites.data.length">
                                <td colspan="4" class="px-6 py-8 text-center text-sm text-gray-500">Aucune specialite trouvee.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                <div v-if="specialites.links?.length" class="border-t border-gray-100 px-4 py-3">
        ...
        <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center px-4">
            <div class="absolute inset-0 bg-ed-primary/40" @click="showModal = false"></div>
            <div class="relative w-full max-w-md rounded-xl bg-white p-6">
                <h3 class="text-lg font-semibold text-gray-900">{{ editingSpecialite ? 'Modifier la specialite' : 'Nouvelle specialite' }}</h3>
                <form class="mt-4 space-y-4" @submit.prevent="submit">
                    <div>
                        ...
                        <input
                            v-model="form.nom"
                            type="text"
                            class="mt-2 w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm focus:border-ed-primary focus:ring-ed-primary/20"
                        />
                        <p v-if="form.errors.nom" class="mt-1 text-xs text-red-600">{{ form.errors.nom }}</p>
                    </div>
                    <div>
                        <label class="text-xs font-semibold text-gray-700">EAD *</label>
                        <select v-model="form.ead_id" class="mt-2 w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm focus:border-ed-primary focus:ring-ed-primary/20">
                    ...
                    <div class="flex justify-end gap-2 pt-4">
                        <button type="button" class="rounded-xl border border-gray-200 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-50" @click="showModal = false">
                            Annuler
                        </button>
