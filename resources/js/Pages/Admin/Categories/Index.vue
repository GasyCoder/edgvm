...
watch(search, () => {
    ...
    searchTimeout = setTimeout(() => {
        router.get(route('admin.categories.index'), { search: search.value || undefined }, {
            preserveState: true,
            preserveScroll: true,
            replace: true,
        });
...
const openEdit = (category) => {
    ...
    form.couleur = category.couleur || '#3B82F6';
    ...
    <AdminLayout>
        ...
        <div class="space-y-6">
            <PageHeader
                badge="Organisation"
                title="Categories"
                description="Gestion des categories d'actualites."
            >
                <template #actions>
                    <button
                        type="button"
        ...
        <div class="overflow-hidden rounded-2xl border border-slate-100 bg-white">
            <table class="min-w-full text-sm">
                ...
                <tbody class="divide-y divide-slate-100">
                    <tr v-for="category in categories.data" :key="category.id" class="hover:bg-slate-50/70">
                        <td class="px-5 py-4">
                            <div class="font-medium text-slate-900">{{ category.nom }}</div>
                            <div v-if="category.description" class="text-xs text-slate-500">
                                {{ category.description }}
                            </div>
                        </td>
                        <td class="px-5 py-4">
                            <span class="inline-block h-6 w-6 rounded-full" :style="{ backgroundColor: category.couleur }"></span>
                        </td>
                        <td class="px-5 py-4 text-slate-500">{{ category.actualites_count }}</td>
                        <td class="px-5 py-4">
                            <span
                                class="rounded-full px-2.5 py-1 text-xs font-semibold"
                                :class="category.visible ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-600'"
                            >
                        ...
                        <td class="px-5 py-4 text-right">
                            <div class="flex justify-end gap-2">
                                <button class="inline-flex items-center gap-1.5 rounded-lg border border-slate-200 px-3 py-1.5 text-xs font-semibold text-slate-700 hover:bg-slate-100" @click="openEdit(category)">
                                ...
                                <button class="inline-flex items-center gap-1.5 rounded-lg border border-red-200 px-3 py-1.5 text-xs font-semibold text-red-600 hover:bg-red-50" @click="removeCategory(category)">
                                    <svg class="h-3.5 w-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <tr v-if="!categories.data.length">
                        <td colspan="5" class="px-6 py-10 text-center text-slate-500">Aucune categorie trouvee.</td>
                    </tr>
                </tbody>
            </table>
        </div>
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center px-4">
        ...
        <div class="relative w-full max-w-lg rounded-2xl bg-white p-6">
            ...
            <form class="mt-4 space-y-4" @submit.prevent="submit">
                ...
                <div>
                    <label class="text-xs font-semibold text-slate-700">Couleur</label>
                    <input v-model="form.couleur" type="color" class="mt-2 h-10 w-20 rounded border border-slate-200" />
                    <p v-if="form.errors.couleur" class="mt-1 text-xs text-red-600">{{ form.errors.couleur }}</p>
                </div>

                <label class="flex items-center gap-2 text-sm">
                    <input v-model="form.visible" type="checkbox" class="rounded border-slate-300 text-ed-primary focus:ring-ed-primary/20" />
                    Visible
                </label>
                ...
                <div class="flex justify-end gap-2 pt-4">
                    ...
                    <button
                        type="submit"
                        class="rounded-xl bg-ed-primary px-4 py-2 text-sm font-semibold text-white hover:bg-ed-secondary disabled:cursor-not-allowed disabled:opacity-60"
