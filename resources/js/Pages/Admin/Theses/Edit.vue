...
const form = useForm({
    doctorant_id: props.these?.doctorant_id || '',
    sujet_these: props.these?.sujet_these || '',
    specialite_id: props.these?.specialite_id || '',
    ead_id: props.these?.ead_id || '',
    universite_soutenance: props.these?.universite_soutenance || '',
    date_debut: props.these?.date_debut || '',
    date_prevue_fin: props.these?.date_prevue_fin || '',
    date_publication: props.these?.date_publication || '',
    statut: props.these?.statut || 'en_cours',
    media_id: props.these?.media_id || '',
    slug: '',
    these_file: null,
    resume_these: props.these?.resume_these || '',
    mots_cles: props.these?.mots_cles || '',
    encadrants: props.these?.encadrants || [],
});
...
onMounted(() => {
    const doctorant = (props.doctorants || []).find((item) => String(item.id) === String(form.doctorant_id));
    if (doctorant) {
        doctorantSearch.value = `${doctorant.name} (${doctorant.matricule})`;
    }
...
const submit = () => {
    form.put(route('admin.theses.update', props.these.id), {
        forceFormData: true,
    });
};
</script>
