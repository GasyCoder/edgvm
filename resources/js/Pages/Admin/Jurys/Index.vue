...
<template>
    <AdminLayout>
        <template #header>
            <h1 class="text-lg font-semibold text-gray-900">Jurys</h1>
        </template>
        ...
        <div class="space-y-4">
            ...
            <div class="grid gap-4 rounded-xl border border-gray-200 bg-white p-4 lg:grid-cols-2">
                <div class="rounded-xl border border-gray-100 bg-gray-50 px-4 py-3">
                    <p class="text-xs text-gray-500">Total jurys</p>
            ...
            <div class="flex flex-wrap items-end gap-3 rounded-xl border border-gray-200 bg-white p-4">
                <div class="flex-1 min-w-[200px]">
                    ...
                    <input v-model="search" type="text" placeholder="Nom, grade, universite" class="mt-2 w-full rounded-xl border border-gray-200 px-3 py-2 text-sm" />
                </div>
                ...
                <div class="overflow-x-auto">
                    <table class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="divide-y divide-gray-100 text-sm">
                            <tr v-for="jury in jurys.data" :key="jury.id" class="hover:bg-gray-50">
                                <td class="px-5 py-4">
                                    <p class="font-semibold text-gray-900">{{ jury.nom }}</p>
                                    <p class="text-xs text-gray-500">{{ jury.grade || '-' }}</p>
                                </td>
                                <td class="px-5 py-4 text-gray-600">{{ jury.universite || '-' }}</td>
                                <td class="px-5 py-4 text-gray-600">
                                    <p>{{ jury.email || '-' }}</p>
                                    <p class="text-xs text-gray-400">{{ jury.phone || '-' }}</p>
                                </td>
                                <td class="px-5 py-4 text-gray-600">
                                ...
                                <td class="px-5 py-4 text-right">
                                    <div class="flex justify-end gap-1">
                                        <Link :href="route('admin.jurys.edit', jury.id)" class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-gray-200 text-gray-600 transition hover:border-ed-primary hover:bg-ed-primary/5 hover:text-ed-primary" title="Modifier">
                                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <button type="button" class="inline-flex h-8 w-8 items-center justify-center rounded-lg border border-gray-200 text-gray-600 transition hover:border-red-300 hover:bg-red-50 hover:text-red-600" title="Supprimer" @click="deleteJury(jury)">
                                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <tr v-if="!jurys.data.length">
                                <td colspan="5" class="px-6 py-8 text-center text-sm text-gray-500">Aucun jury trouve.</td>
                            </tr>
                        </tbody>
                    </table>
                </div>
                <div v-if="jurys.links?.length" class="border-t border-gray-100 px-4 py-3">
