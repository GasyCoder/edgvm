...
const submit = () => {
    form.post(route('admin.jurys.store'));
};
</script>
    ...
    <AdminLayout>
        <template #header>
            <div class="flex flex-wrap items-end justify-between gap-4">
                <div>
                    <h2 class="text-lg font-semibold text-slate-900 md:text-xl">Nouveau jury</h2>
                    <p class="mt-1 text-xs text-slate-500 md:text-sm">Ajoutez un membre de jury.</p>
                </div>
        ...
        <form class="grid gap-6 lg:grid-cols-2" @submit.prevent="submit">
            ...
            <section class="rounded-2xl border border-dashed border-slate-200 bg-slate-50 p-5 text-xs text-slate-600">
                Astuce: renseignez l'universite pour un affichage clair dans les rapports.
            </section>
        </form>
