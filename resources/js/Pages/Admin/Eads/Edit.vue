...
const form = useForm({
    nom: props.ead?.nom || '',
    sigle: props.ead?.sigle || '',
    description: props.ead?.description || '',
    responsable_id: props.ead?.responsable_id || '',
    encadrants: props.ead?.encadrants || [],
});

const selectedEncadrantId = ref('');

const submit = () => {
    form.put(route('admin.ead.update', props.ead.slug));
};
...
<template>
    <AdminLayout>
        <template #header>
            <h2 class="text-lg font-semibold text-slate-900 md:text-xl">Modifier l'équipe d'accueil</h2>
        </template>

        <Head title="Modifier l'équipe d'accueil" />
        <FlashMessage />
        ...
        <div class="mx-auto max-w-screen-2xl space-y-6">
            <nav class="flex flex-wrap items-center gap-2 text-xs text-gray-500">
                ...
                <span aria-hidden="true">/</span>
                <span class="font-semibold text-gray-900">{{ ead.nom }}</span>
            </nav>
            ...
            <form class="grid gap-6 lg:grid-cols-3" @submit.prevent="submit">
                <section class="space-y-5 rounded-xl border border-gray-200 bg-white p-6 lg:col-span-2">
                    <div>
                        ...
                        <input v-model="form.nom" type="text" class="w-full rounded-lg border border-gray-300 px-3.5 py-2.5 text-sm placeholder:text-gray-400 focus:border-transparent focus:ring-2 focus:ring-ed-primary/40">
                        <p class="mt-1 text-xs text-gray-500">Le lien public (slug) est régénéré automatiquement à partir du nom.</p>
