...
const selectedActualite = ref(props.actualitePreview ?? null);
...
const form = useForm({
    titre_ligne1: props.slide?.titre_ligne1 ?? '',
    titre_highlight: props.slide?.titre_highlight ?? '',
    titre_ligne2: props.slide?.titre_ligne2 ?? '',
    description: props.slide?.description ?? '',
    image_id: props.slide?.image_id ?? null,
    new_image: null,
    lien_cta: props.slide?.lien_cta ?? '',
    actualite_id: props.slide?.actualite_id ?? null,
    texte_cta: props.slide?.texte_cta ?? '',
    ordre: props.slide?.ordre ?? 1,
    visible: props.slide?.visible ?? true,
    badge_texte: props.slide?.badge_texte ?? '',
    badge_icon: props.slide?.badge_icon ?? 'university',
    couleur_fond: props.slide?.couleur_fond ?? backgroundOptions[0].value,
});
...
const applySearch = () => {
    router.get(route('admin.slides.edit', { slider: props.slider.id, slide: props.slide.id }), {
...
const submit = () => {
    form.put(route('admin.slides.update', { slider: props.slider.id, slide: props.slide.id }), {
        preserveScroll: true,
        forceFormData: true,
    });
};
                    ...
                    <section class="rounded-2xl border border-slate-100 bg-white p-6">
                        ...
                        <div class="mt-4 space-y-4">
                            ...
                            <div class="flex items-center justify-between gap-4 rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 py-6">
                                <div>
                                    <p class="text-sm font-semibold text-slate-700">Uploader une nouvelle image</p>
                    ...
                    <section class="rounded-2xl border border-slate-100 bg-white p-6">
                        ...
                        <div class="mt-4 space-y-4">
                            ...
                            <div>
                                ...
                                <input v-model="form.lien_cta" type="text" class="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:border-ed-primary focus:ring-ed-primary/20" />
                            </div>
                            <div>
                                <label class="text-xs font-semibold text-slate-700">Texte CTA</label>
                                <input v-model="form.texte_cta" type="text" class="mt-2 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm focus:border-ed-primary focus:ring-ed-primary/20" />
                            </div>
                        </div>
                    </section>
                </div>
