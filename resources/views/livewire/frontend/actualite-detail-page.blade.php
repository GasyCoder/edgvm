                    ...
                    <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
                        ...
                        <div class="p-6 md:p-10">
                            <div class="prose prose-gray max-w-none
                                        ...
                                        prose-a:text-ed-primary prose-a:no-underline hover:prose-a:underline 
                        ...
                        <div class="px-6 md:px-10 pb-6">
                            <div class="bg-gray-50 rounded-xl p-6">
                                <h3 class="text-lg font-bold text-gray-900 mb-4 flex items-center gap-2">
                                    ...
                                    Galerie photos
                                    <span class="text-sm text-gray-500 font-normal">({{ $actualite->galerie->count() }})</span>
                                </h3>
                                
                                <!-- Carousel -->
                                <div x-data="{ currentSlide: 0, totalSlides: {{ $actualite->galerie->count() }} }">
                                    ...
                                    <div class="relative rounded-xl overflow-hidden shadow-lg mb-4 group">
                                        ...
                                        <!-- Compteur -->
                                        <div class="absolute bottom-3 left-1/2 -translate-x-1/2 px-3 py-1 bg-black/50 backdrop-blur-sm text-white rounded-full text-xs font-semibold">
                                    ...
                                    <div class="grid grid-cols-5 sm:grid-cols-7 gap-2">
                                        ...
                                        <button @click="currentSlide = {{ $index }}"
                                                :class="currentSlide === {{ $index }} ? 'ring-2 ring-ed-primary' : 'opacity-60 hover:opacity-100'"
                        ...
                        <div class="px-6 md:px-10 pb-6">
                            ...
                            <div class="flex flex-wrap gap-2">
                                <a href="https://www.facebook.com/sharer/sharer.php?u={{ urlencode(url()->current()) }}" 
                                   target="_blank"
                                   class="px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg text-xs font-semibold transition flex items-center gap-1.5">
                                ...
                                <a href="https://twitter.com/intent/tweet?url={{ urlencode(url()->current()) }}&text={{ urlencode($actualite->titre) }}" 
                                   target="_blank"
                                   class="px-3 py-2 bg-sky-500 hover:bg-sky-600 text-white rounded-lg text-xs font-semibold transition flex items-center gap-1.5">
                                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                                ...
                                <a href="https://www.linkedin.com/shareArticle?mini=true&url={{ urlencode(url()->current()) }}&title={{ urlencode($actualite->titre) }}" 
                                   target="_blank"
                                   class="px-3 py-2 bg-blue-700 hover:bg-blue-800 text-white rounded-lg text-xs font-semibold transition flex items-center gap-1.5">
                                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                                ...
                                <a href="https://wa.me/?text={{ urlencode($actualite->titre . ' - ' . url()->current()) }}" 
                                   target="_blank"
                                   class="px-3 py-2 bg-green-500 hover:bg-green-600 text-white rounded-lg text-xs font-semibold transition flex items-center gap-1.5">
                                    <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="px-6 md:px-10 pb-6 pt-6 border-t border-gray-100">
                            <div class="flex justify-between gap-4">
                                ...
                                <a href="{{ route('actualites.show', $precedent) }}" 
                                   class="group flex items-center gap-2 text-ed-primary hover:text-ed-secondary transition">
                                    <svg class="w-5 h-5 group-hover:-translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/>
                                    </svg>
                                    <span class="text-sm font-semibold">Article précédent</span>
                                </a>
                                @else
                                <div></div>
                                @endif
                    ...
                    <div class="lg:sticky lg:top-24 space-y-6">
                        ...
                        <div class="bg-gradient-to-br from-ed-primary via-ed-secondary to-purple-600 rounded-2xl shadow-xl overflow-hidden">
                            <div class="p-6 text-white">
                                <div class="text-center mb-4">
                                    ...
                                    <p class="text-white/80 text-xs">Recevez nos actualités</p>
                                </div>
                                ...
                                <form wire:submit="subscribe" class="space-y-2.5">
                                    <input type="email" 
                                           ...
                                           placeholder="Email"
                                           class="w-full px-3 py-2.5 text-sm rounded-lg bg-white/20 backdrop-blur-md border border-white/30 text-white placeholder-white/70 focus:ring-2 focus:ring-white transition"
                                           required>
                                    ...
                                    <select wire:model="newsletterType"
                                            class="w-full px-3 py-2.5 text-sm rounded-lg bg-white text-gray-900 border border-white/30 focus:ring-2 focus:ring-ed-yellow font-semibold">
                        ...
                        <div class="bg-white rounded-2xl shadow-xl overflow-hidden">
                            <div class="p-4 border-b border-gray-100">
                                <h3 class="text-base font-bold text-gray-900">Articles similaires</h3>
                            </div>
                            
                            <div class="p-4 space-y-3">
                                @foreach($similaires as $similaire)
                                <a href="{{ route('actualites.show', $similaire) }}" 
                                   class="group block p-2 rounded-lg hover:bg-gray-50 transition">
                                    <div class="flex gap-3">
                                        @if($similaire->image)
                                        <img src="{{ $similaire->image->url }}" 
                                             class="w-20 h-20 rounded-lg object-cover flex-shrink-0 group-hover:scale-105 transition-transform"
                                             alt="{{ $similaire->titre }}">
                                        @else
                                        <div class="w-20 h-20 rounded-lg bg-gradient-to-br from-ed-primary to-ed-secondary flex-shrink-0"></div>
                                        @endif
                                        
                                        <div class="flex-1 min-w-0">
                                            <h4 class="text-sm font-semibold text-gray-900 group-hover:text-ed-primary line-clamp-2 mb-1 transition">
                                                {{ $similaire->titre }}
                                            </h4>
                                            <p class="text-xs text-gray-500 mb-1">
                                                {{ $similaire->date_publication->locale('fr')->isoFormat('D MMM YYYY') }}
                                            </p>
